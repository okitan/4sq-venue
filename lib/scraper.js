"use strict";

const puppeteer = require("puppeteer");

const { phoneExtractor } = require("./util");
const { createScrapedVenue } = require("./venue");

const scrape = async (url, venues, options, notify) => {
  const results = [];

  let browser;
  try {
    browser = await puppeteer.launch({
      headless: process.env.NO_HEADLESS ? false : true,
      args: ["--no-sandbox", "--disable-setuid-sandbox"],
    });

    const page = await browser.newPage();
    await page.setExtraHTTPHeaders({ "Accept-Language": "en" }); // umhh...
    await page.goto(url, options);

    for (const [selector, { followLink, ...properties }] of Object.entries(venues)) {
      const items = await page.$$(selector);

      if (items.length === 0) {
        throw new Error(`no venues found for ${selector}`);
      }

      // console.error("\n", detail);
      for (const item of items) {
        if (followLink) {
          const subVenueUrl = await scrapeVenueValue(item, followLink);

          if (notify) notify(`Scraping subvenue ${subVenueUrl}`);

          let newPage;
          try {
            newPage = await browser.newPage();
            await newPage.goto(subVenueUrl, options);

            const result = await scrapeVenue(newPage, properties);
            result.scraped.url = subVenueUrl;

            results.push(result);
          } catch (err) {
            throw new Error(`on scraping ${subVenueUrl}: ${err.message}`, err);
          } finally {
            if (newPage) await newPage.close();
          }
        } else {
          results.push(await scrapeVenue(item, properties));
        }
      }
    }
  } finally {
    if (browser) await browser.close();
  }

  return results;
};

const scrapeVenue = async (venue, properties) => {
  const results = {};

  for (const [key, value] of Object.entries(properties)) {
    let result = await scrapeVenueValue(venue, value);

    if (key === "phone" && result) {
      result = phoneExtractor(result);
    }

    results[key] = result;
  }

  return createScrapedVenue(results);
};

const scrapeVenueValue = async (venue, handler) => {
  switch (typeof handler) {
    case "object":
      const { selector, xpath, property = "innerText", modifier, nullable } = handler;

      const element = await (async () => {
        if (selector) {
          return await venue.$(selector);
        } else if (xpath) {
          return (await venue.$x(xpath))[0];
        } else {
          throw new Error(`neither selector nor xpath defined: ${handler}`);
        }
      })();

      if (!element) {
        if (nullable) {
          return;
        } else {
          throw new Error(`no elements found for ${selector || xpath}`);
        }
      }

      const result = (await (await element.getProperty(property)).jsonValue())
        .replace(/[Ａ-Ｚａ-ｚ０-９]/g, function (s) {
          return String.fromCharCode(s.charCodeAt(0) - 65248);
        })
        .replace(/\s/g, " ") // node includes 全角スペース to \s
        .replace(/ +/g, " ")
        .trim();

      return modifier ? modifier(result) : result;
    default:
      return handler;
  }
};

module.exports = {
  scrape,
};
