"use strict";

module.exports = {
  command: "scrape <target> [Options]",
  desc: "scrape",
  builder: {}
};

const fs = require("fs");

const ora = require("ora");

const { updateVenues } = require("../venueList");
const { scrape } = require("../scraper");

module.exports.handler = async argv => {
  const configFile = `../../venues/${argv.target}/config.js`;

  // TODO: error handling
  const config = require(configFile);
  const targets = config.scraper || [];

  const results = [];
  // scrape not in pararelle
  for (const { url, venues } of targets) {
    let spinner;
    try {
      // start scraping
      spinner = ora(`Scraping ${url}`).start();

      const result = await scrape(url, venues, message => {
        spinner.text = [`Scraping ${url}`, message].join("\n");
      });

      results.push(...result);

      spinner.succeed(
        [`Scraped ${url}`, `${result.length} venues found`].join("\n")
      );
    } catch (err) {
      if (spinner) spinner.fail([`Scrape ${url}`, err].join("\n"));
      throw err;
    }
  }

  updateScrapedVenues(argv.target, results);
};
