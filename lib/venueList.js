...
const venueSorter = (a, b) => {
  // 1st sort by level or crossStreet
  if (a["scraped.level"] < b["scraped.level"]) {
    return -1;
  } else if (a["scraped.level"] > b["scraped.level"]) {
    return 1;
  }
  if (a.crossStreet < b.crossStreet) {
    return -1;
  } else if (a.crossStreet > b.crossStreet) {
    return 1;
  }

  // 2nd sort by name
  if (a["scraped.name"] < b["scraped.name"]) {
    return -1;
  } else if (a["scraped.name"] > b["scraped.name"]) {
    return 1;
  }
  if (a.name < b.name) {
    return -1;
