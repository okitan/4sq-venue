...
class VenueList extends Array {
  findVenueIndex(venue, { guess = false } = {}) {
    const index = this.findIndex(e => venue.equals(e));

    if (index > -1) return index;

    if (guess) {
      const scores = this.map(e => venue.getSimilarityOfName(e));
  ...
  findVenue(venue, { guess = false } = {}) {
    return this[this.findVenueIndex(venue, { guess })];
  }

  // returns removed venue
  removeVenue(venue, { guess = false } = {}) {
    return this.splice(this.findVenueIndex(venue, { guess }), 1)[0];
  }
}
