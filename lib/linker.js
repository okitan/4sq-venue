...
const linkVenues = (scrapedVenues, foursquareVenues, previousResult) => {
  ...
  // check previous result still exists on scraped result
  previousResult.forEach(result => {
    const scrapedVenue = findVenue(scrapedVenues, result, { remove: true });

    if (scrapedVenue) {
      // update previous result
      result.scraped = scrapedVenue.scraped;

      linkedVenues.push(result);
      findVenue(foursquareVenues, result, { remove: true });
    }
  });
