...
const linkVenues = (
  ...
  matchVenues(scrapedVenues, foursquareVenues, { ignore }).forEach(
    ({ scrapedVenue, foursquareVenue }) => {
  ...
  // create guess map
  guessVenues(scrapedVenues, foursquareVenues, { ignore }).forEach(
    ({ scrapedVenue, foursquareVenue }) => {
...
const guessVenues = (scrapedVenues, foursquareVenues, { ignore } = {}) => {
  return flatmap(scrapedVenues, scrapedVenue => {
    ...
    .filter(({ scrapedVenue, foursquareVenue }, _, array) => {
      const duplicates = array.filter(e =>
        foursquareVenue.equals(e.foursquareVenue)
      );
      ...
      if (duplicates.length > 1) {
        ...
        return duplicates
          .find(({ score }) => score === best)
          .scrapedVenue.equals(scrapedVenue);
      } else {
        return true;
      }
    });
